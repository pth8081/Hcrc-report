// scripts/seedAdmin.js — Tạo tài khoản quản trị ĐẦU TIÊN cho etl-admin/.
// Cách dùng:
//   node scripts/seedAdmin.js <username> <password> "<Họ tên>" [admin|viewer|target_importer]
// role mặc định "admin" nếu không truyền — cột AdminUsers.Role CHỈ còn dùng
// để hiển thị/lịch sử (xem etl-db/schema.sql), quyền THẬT nay tra qua
...
async function main() {
  ...
  if (!username || !password || !fullName) {
    console.error('Cách dùng: node scripts/seedAdmin.js <username> <password> "<Họ tên>" [admin|viewer|target_importer]');
    process.exit(1);
  }
  if (!['admin', 'viewer', 'target_importer'].includes(role)) {
    console.error('Role phải là "admin", "viewer" hoặc "target_importer".');
    process.exit(1);
  }
