...
router.post('/login', async (req, res, next) => {
  try {
    ...
    // twoFactor.js) — 'viewer'/'target_importer' KHÔNG áp dụng, đăng nhập
    // xong ngay như trước đây. Đúng mật khẩu nhưng CHƯA qua đủ 2 yếu tố ->
    // KHÔNG đặt cookie phiên đầy đủ, chỉ trả token trung gian.
    if (admin.role === 'admin') {
