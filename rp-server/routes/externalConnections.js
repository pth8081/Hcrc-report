...
const AUTH_TYPES = ['none', 'headerKey', 'queryParam', 'basicAuth'];

function validateAuthPayload({ authType = 'none', authKeyName, authValue, authUsername, authPassword }) {
  if (!AUTH_TYPES.includes(authType)) return `authType phải là một trong: ${AUTH_TYPES.join(', ')}`;
  if ((authType === 'headerKey' || authType === 'queryParam') && (!authKeyName || !authValue)) {
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const result = await pool.request().query(`
      SELECT Id, Name, BaseUrl, AuthType, AuthKeyName, AuthUsername, CreatedAt
...
router.post('/', async (req, res, next) => {
  try {
    const { name, baseUrl, authType = 'none', authKeyName, authValue, authUsername, authPassword } = req.body || {};
    if (!name || !baseUrl) return res.status(400).json({ error: 'Thiếu name/baseUrl' });
    const authError = validateAuthPayload({ authType, authKeyName, authValue, authUsername, authPassword });
    ...
    const result = await pool.request()
      ...
      .input('authType', sql.VarChar(20), authType)
      .input('authKeyName', sql.NVarChar(200), authType === 'headerKey' || authType === 'queryParam' ? authKeyName : null)
      .input('authValueEncrypted', sql.NVarChar(500), authType === 'headerKey' || authType === 'queryParam' ? encrypt(authValue) : null)
      ...
      .query(`
        INSERT INTO app.ExternalApiConnections (Name, BaseUrl, AuthType, AuthKeyName, AuthValueEncrypted, AuthUsername, AuthPasswordEncrypted)
        OUTPUT INSERTED.Id
        VALUES (@name, @baseUrl, @authType, @authKeyName, @authValueEncrypted, @authUsername, @authPasswordEncrypted)
      `);
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    if (authType === 'headerKey' || authType === 'queryParam') {
      if (authValue) {
    ...
    await pool.request()
      ...
      .query(`
        ...
        SET Name = @name, BaseUrl = @baseUrl, AuthType = @authType, AuthKeyName = @authKeyName,
            AuthValueEncrypted = @authValueEncrypted, AuthUsername = @authUsername, AuthPasswordEncrypted = @authPasswordEncrypted
        WHERE Id = @id
      `);
