// lib/twoFactor.js — Xác thực hai yếu tố (TOTP, RFC 6238) cho user rp-user/
// có vai trò IsSystemRole=1 ("Admin", xem lib/permissions.js) — BẮT BUỘC, xem
// routes/twoFactor.js + lib/auth.js. Secret mã hoá bằng lib/crypto.js
// (RP_ENCRYPTION_KEY), KHÔNG lưu plaintext trong CSDL. Mã khôi phục (10 mã
// dùng 1 lần) hash bằng bcrypt, hiện nguyên văn cho user đúng 1 LẦN lúc bật
// 2FA — sau đó không đọc lại được, chỉ đối chiếu hash.
//
// Bản sao CÙNG NỘI DUNG cũng có ở etl/lib/ và api-server/lib/ — cố ý trùng
...
// trong CSDL (app.Users.TwoFactorSecretEncrypted). epochTolerance: 1 -> chấp
// nhận CẢ bước 30s TRƯỚC/SAU thời điểm máy chủ, bù lệch đồng hồ nhỏ giữa
// điện thoại và máy chủ (thực hành chuẩn cho TOTP — hầu hết ứng dụng
