...
//
// CHỈ dùng cho đường HTTP tương tác (routes/v1/reports.js:/:reportId/run).
//
// Cache CHUNG cho mọi đối tác gọi (không phân biệt consumer) — an toàn vì
// kết quả chỉ phụ thuộc reportId+filters+trang+fields, không phụ thuộc danh
// tính đối tác (quyền GỌI được báo cáo nào đã kiểm tra RIÊNG qua
// api.ConsumerReportAccess, trước khi tới cache này — cache không làm lộ
// dữ liệu cho đối tác không có quyền, chỉ tiết kiệm việc CHẠY LẠI truy vấn
// khi 2 đối tác CÙNG được phép và CÙNG hỏi y hệt tham số).
//
// Bản sao CÙNG NỘI DUNG cũng có ở rp-server/lib/ — cố ý trùng lặp, theo
